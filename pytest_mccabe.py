...
def pytest_sessionstart(session):
    ...
    if config.option.mccabe:
        config._mccabe_complexities = Complexities(config.getini("mccabe-complexity"))
...
class McCabeItem(pytest.Item, pytest.File):
    ...
    def runtest(self):
        ...
        self.config._mccabe_mtimes[str(self.fspath)] = (self._mtime, self.complexity)
    ...
    def reportinfo(self):
        # FIXME
        #if self.complexity:
        #    ignores = "(ignoring %s)" % " ".join(self.complexity)
        #else:
        ignores = ""
        return (self.fspath, -1, "mccabe-check%s" % ignores)
...
def check_file(path, complexity):
    ...
    try:
        ...
        for lineno, offset, text, _checker in checker.run():
